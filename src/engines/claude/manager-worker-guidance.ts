export function buildManagerWorkerGuidance(): string {
  return [
    ...
    'Worker tasks are asynchronous and traceable. After dispatching, use get_worker_task or list_worker_tasks to inspect status, event history, results, cost, and errors. Summarize results back to the user with task IDs, evidence, risks/gaps, and next steps. Use schedule_reminder for follow-ups that must survive MetaBot restarts.',
  ].join('\n');
}
