...
function ctx(service: any): RouteContext {
  return { managerService: service } as RouteContext;
}
...
function service() {
  return {
    ...
    getTask: vi.fn(() => ({
      id: 'mgrtask-1', traceId: 'trace-1', managerBotName: 'manager', managerChatId: 'chat-a',
      workerBotName: 'worker-a', workerChatId: 'manager-worker-abc', prompt: 'do it', status: 'completed',
...
describe('manager routes', () => {
  it('lists workers with manager scope query params', async () => {
    const svc = service();
    const out = res();
    const handled = await handleManagerRoutes(ctx(svc), req(), out, 'GET', '/api/manager/workers?managerBotName=manager&managerChatId=chat-a');
  ...
  it('gets task details with events', async () => {
    const svc = service();
    const out = res();
    await handleManagerRoutes(ctx(svc), req(), out, 'GET', '/api/manager/tasks/mgrtask-1?managerBotName=manager&managerChatId=chat-a&includeEvents=true');
    expect(out.statusCode).toBe(200);
    expect(out.body.task.id).toBe('mgrtask-1');
  ...
  it('schedules and cancels reminders', async () => {
    ...
    const createOut = res();
    await handleManagerRoutes(ctx(svc), req({
      managerBotName: 'manager', managerChatId: 'chat-a', prompt: 'remember', delaySeconds: 60,
    }), createOut, 'POST', '/api/manager/reminders');
    expect(createOut.statusCode).toBe(201);
    expect(createOut.body.reminder.id).toBe('sched-1');

    const deleteOut = res();
    await handleManagerRoutes(ctx(svc), req(), deleteOut, 'DELETE', '/api/manager/reminders/sched-1?managerBotName=manager&managerChatId=chat-a');
